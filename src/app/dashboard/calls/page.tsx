...
import { getCalls, getCall, type CallDetail } from "@/lib/dashboard/data";
import { CallsView } from "./CallsView";

const GS = "var(--font-geist-sans)";

export default async function CallsPage() {
  // Call history: list + full detail (transcript + tool trace) for each call.
  const list = await getCalls();
  const details = (await Promise.all(list.map((c) => getCall(c.id)))).filter(
      ...
      <div className="flex items-end justify-between px-9 pt-9 pb-6 shrink-0" style={{ borderBottom: "1px solid #20183C" }}>
        <div>
          ...
          <p className="mt-2 text-sm" style={{ color: "#8C86A8" }}>
            Every enquiry Mello handled · transcripts kept, audio never stored
          </p>
        </div>
